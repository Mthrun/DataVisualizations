DensityScatter=function(X,Y,DensityEstimation="SDH",SampleSize,na.rm=FALSE,PlotIt=TRUE,
                        ...
                        xlim, ylim, pointSize=1, Legendlab_ggplot="value",AddString2lab="",NoBinsOrPareto=NULL,cls=NULL,...){
  # DensityScatter
  ...
  if(clsColoring) {
    if(length(cls) != length(X))
      stop('DensityScatter: cls has to have the same length as the input data')
    if(any(!is.numeric(cls) | is.nan(cls)))
      stop('DensityScatter: cls has to be a numeric vector without NaN values')
  }
  ...
  if(DrawTopView){
    ...
    switch(Plotter,'ggplot'={
      ...
      if(clsColoring) {
        # Check if the amount of labels does not correspond, to the amount of colors
        lengthsFits = TRUE
        colpal <- cut(cls, length(cls), labels = FALSE)
        cols[noNaNInd] <- colpalette(length(cls))[colpal]
      } else {
      ...
      plot(X[noNaNInd],Y[noNaNInd],col=cols,pch=".",xlim = xlim,
           ylim = ylim,xlab=xlab,ylab=ylab,main=main,cex=pointSize,...)
